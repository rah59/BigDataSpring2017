...
object IPApp {
  ...
  val IMAGE_CATEGORIES = List("S1", "S2", "S3", "S4", "S5")
  ...
  def main(args: Array[String]) {
    ...
    val sparkConf = new SparkConf().setAppName("SparkFaceRecognition").setMaster("local[*]")

    val sc=new SparkContext(sparkConf)

    val images = sc.wholeTextFiles(s"${IPSettings.INPUT_DIR}/*/*.tif")
    ...
    val testImages = sc.wholeTextFiles(s"${IPSettings.TEST_INPUT_DIR}/*/*.tif")
