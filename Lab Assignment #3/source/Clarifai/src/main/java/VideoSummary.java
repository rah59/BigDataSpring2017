 ...
 * Adopted by Raj on 02-7-17 to create a video summary using OpenIMAJ and Clarifai
 * The idea is to grab the keyframes from the video and then pass it to clarifai to get the annotations for each key frame
 *
 */
public class VideoSummary {
    ...
    public static void MainFrames(){
        ...
        System.out.println("Reviewing the keyframes indicates that the video can be best described using the keywords");
    ...
    public static void GetSummary(String filename) {
        ...
        ClarifaiResponse response = client.getDefaultModels().generalModel().predict()
                .withInputs(
                        ClarifaiInput.forImage(ClarifaiImage.of(new File(filename)))
                )
                .executeSync();
